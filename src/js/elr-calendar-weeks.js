...
const elrCalendarWeeks = function() {
  ...
  let createWeekHtml = function(datesInWeek, weekNum) {
    ...
    let weekHtml = `<tbody class="${
      classes.week
    } ${weekClass}" data-week="${weekNum}">`
  ...
  let createWeekHeading = function(dateObj, datesInWeek, weekNum) {
    ...
    let endDate = {
      month:
        elrTime.months[
          datesInWeek.datesInWeek[datesInWeek.datesInWeek.length - 1].month
        ],
      date: datesInWeek.datesInWeek[datesInWeek.datesInWeek.length - 1].date
    }
    let year = dateObj.year
    let text = `${startDate.month} ${startDate.date} - ${endDate.month} ${
      endDate.date
  ...
  let self = {
    renderWeek: function(dateObj, $cal, evts) {
      ...
      let calendarHtml = $('<div></div>', {
        class: 'calendar-inner elr-calendar-week-view',
      ...
      $heading.prependTo($cal.find(`.elr-calendar-week-view`))

      // add and remove navigation buttons
      $('.elr-calendar-year-prev')
        .text(lastYear)
        .hide()
      $('.elr-calendar-year-next')
        .text(nextYear)
        .hide()

      $('.elr-calendar-month-prev')
        .text(elrTime.months[lastMonth])
        .hide()
      $('.elr-calendar-month-next')
        .text(elrTime.months[nextMonth])
        .hide()
